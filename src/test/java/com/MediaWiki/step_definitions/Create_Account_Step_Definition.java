...
public class Create_Account_Step_Definition {
	...
	@And("^I enter registration credentials in the \"([^\"]*)\", \"([^\"]*)\", \"([^\"]*)\"$")
	public void i_enter_registration_credentials_in_the(String username, String password, String confirm_password) {
		homePage.userName.sendKeys(username);
		homePage.password.sendKeys(password);
		homePage.confirmPassword.sendKeys(confirm_password);
		homePage.createYourAccount.click();
	}

	@Then("^I should get failure message \"([^\"]*)\"$")
	public void i_should_get_failure_message(String message) {
